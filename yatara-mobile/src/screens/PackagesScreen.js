  ...
  const handleSave = async () => {
    ...
    try {
      const payload = {
        ...
        destinations: [], // Simplified for mobile UI
      };

      if (editingId) {
        await client.patch(`/packages/${editingId}`, payload);
  ...
  const renderItem = ({ item }) => (
    <View style={styles.card}>
      <View style={styles.cardContent}>
        ...
        <Text style={styles.text}>LKR {item.price} • {item.durationDays} Days</Text>
      </View>
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        <Text style={styles.title}>Packages (Module 2)</Text>
      ...
      {loading ? (
        ...
        <FlatList
          ...
          ListEmptyComponent={<Text style={{color: '#fff'}}>No packages found.</Text>}
        />
      )}
      ...
      <Modal visible={modalVisible} animationType="slide" transparent={true}>
        <View style={styles.modalOverlay}>
          <View style={styles.modalView}>
            <Text style={styles.modalTitle}>{editingId ? 'Edit Package' : 'Create Package'}</Text>
