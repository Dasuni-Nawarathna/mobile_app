...
- [2026-05-04] The error "Cannot find module '../encodings'" in `iconv-lite` triggered by `body-parser` (internal to Express 5) can crash the API with a 400 status during JSON body parsing. This often happens in environments where `iconv-lite` dynamic loading fails or when `node_modules` are corrupted/inconsistent across OSs. → Downgrade `express` to stable 4.x or ensure `iconv-lite` version is locked and `node_modules` are clean.

---

## Recent Findings

- [2026-05-04] The login error reported by the user shows Windows paths in the stack trace while the environment is Mac. This confirms the mobile app build or the API server is running on a Windows machine (Dasun's) while being viewed on a Mac.
- [2026-05-04] The `Cannot find module '../encodings'` error is coming from `iconv-lite` inside `body-parser`. It surfaces as a 400 Bad Request because Express catches it during body parsing.
...
- Expo Router + auth gate: keep `app/index.tsx` on `/` for redirect-only logic and put the main tab shell on a named route like `app/(tabs)/home.tsx` (`/home`) so you never duplicate two `index` files that both claim `/`.
- Mobile API base URL should flow from `EXPO_PUBLIC_API_URL` first, then `expo.extra.apiUrl`, then platform defaults (`10.0.2.2` for Android emulator → host `localhost:5000`).
- Expo SDK 54: install native modules with **`npx expo install <pkg>`** — do not hand-pick `expo-secure-store@55.x` (wrong major); SDK 54 expects `expo-secure-store~15.x`. On web, SecureStore’s native module is empty → use a `localStorage` fallback (see `mobile/Yatara-Ceylon/lib/session-storage.ts`).
- [2026-05-04] Role-based navigation pattern: use role helpers (isTourist, isServiceProvider, isAdminOrStaff) in AppNavigator to select the correct Tab.Navigator without any conditionals inside a single Tab group. Each role family gets its own dedicated TabNavigator component.
- [2026-05-04] `authorize(...roles)` middleware usage: always chain as `protect, authorize('ADMIN')` — protect attaches req.user, authorize checks req.user.role. Never call authorize without protect first.
- [2026-05-04] Role-aware register: TOURIST → status=ACTIVE immediately (can use app). DRIVER/HOTEL_MANAGER → status=PENDING_APPROVAL (sees pending banner, cannot access features). Admin reviews in AdminControlPanel and calls PATCH /api/users/:id/approve.
- [2026-05-04] Role validation error: The mobile app could occasionally send role strings with trailing spaces (e.g., `'TOURIST '`), causing backend enum validation to fail. Always use `role.trim()` in AuthContext or the request layer when sending role-based enums to the API.
- [2026-05-04] Public/Guest Mode Pattern: To allow browsing without login, use a Stack Navigator as the PublicRoot that contains a Tab Navigator (GuestTabs) for browsing and a Stack Screen (Login) for authentication. GuestTabs now include Home, Explore, and Account to maximize engagement before login.
- [2026-05-04] Full-Screen UI: For premium immersive designs, disable the default navigator header (`headerShown: false`) in Tab Navigators and handle navigation titles or custom headers within each screen's layout.
...
- [2026-05-04] FlatList Typing: Use the `ListRenderItem<T>` generic type for the `renderItem` function to ensure the `item` parameter is correctly typed within the render block.
- [2026-05-04] Axios Request Interceptors: When typing axios request interceptors in TypeScript, use `InternalAxiosRequestConfig` for the `config` object to match newer Axios versions' expectations.
