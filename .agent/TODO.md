...
**Date**: 2026-05-02
**What was done**:
- Built out `mobile/Yatara-Ceylon` as the official companion app for the standalone Express backend: `AuthProvider` + login/register flows, persisted session via `expo-secure-store`, centralized `lib/api/client` against `EXPO_PUBLIC_API_URL` / dev defaults (`10.0.2.2:5000` on Android emu, localhost on simulator).
- Added tab UX: branded Home (API health ping), stack-based Journeys (GET `/api/packages`, detail + CTA → booking draft), nested Bookings stack (GET `/api/bookings` + POST create with validation), Settings (logout + visible API URL).
- Avoided expo-router `/` duplication by keeping `/` strictly as auth gate (`app/index.tsx`) and renaming the tabs landing screen to `home.tsx` so `/home` is the concierge dashboard shell.

**What to do next**:
- Deploy the Express backend and point `EXPO_PUBLIC_API_URL` at the live origin; widen backend CORS for Expo tunnel hosts if testing on-device.
- When Mongo models ship in `backend/`, mobile list/detail/booking flows will hydrate real data automatically without UI changes besides richer fields.

**Current state**:
- Mobile: Expo SDK 54, `npm install` + `expo-secure-store@^55` added; `npx tsc --noEmit` + `expo lint` clean for the new files.
- Backend: still returns mock entities for several modules (unchanged) — mobile is integration-ready.

**Files changed**:
- `mobile/Yatara-Ceylon/**` (new screens, API layer, auth context, brand tokens, `app.json` extra hook)
